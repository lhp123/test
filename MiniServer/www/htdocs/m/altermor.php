...
?>
	...
	var auth = '<?php echo $_SESSION["auth"] ?>';
		...
		$.post("action.php",{action:"getadmindetail",id:list_id,type:"2"},function(data){
			//alert(data);
...
$("#pre_btn1").click(function(){
	...
	window.open("preview.php?id="+list_id+"&type=2");;
...
$("#tj_btn1").click(function(){
			...
			if(title==""||content==""||btnwords==""||tel==""){
				...
				//alert(op+"");
				var data = {title:title,content:content,commission:commission,img:JSON.stringify(op),state:state,att:"",pid:pid,comm2:comm2,btnwords:btnwords,tel:tel};
				alter_mor(data);
			}
		});
</script>
