...
class ModelAction extends EallAction{
    ...
    public function lists(){
        ...
        if($parent!="-1"){
        	...
        	$listPage =  $this->listPage("a.NAME,a.ID,a.LINK,b.NAME PARENT","a.TABORDER DESC",$where,10,"XT_MODEL a ,XT_MODEL b");
        }else{
        	$listPage =  $this->listPage("a.NAME,a.ID,a.LINK, '' PARENT","a.TABORDER DESC",$where,10,"XT_MODEL a ");
        }
    ...
    public function  getParents($sy=false){
    	$arr = $this->pri->getPri();
    	$this->sql = "SELECT ID ,NAME,PRINAME FROM XT_MODEL  WHERE CID='".$this->getCID()."'  AND LEVEL =1 ";
		if(!$sy){			
			$this->sql = "SELECT ID ,NAME,PRINAME FROM XT_MODEL  WHERE CID='".$this->getCID()."' AND  POSITION(PRINAME IN '".$arr["pri1"]."') > 0  AND LEVEL =1 ";
		}
...
//     			var_dump($arrMenuLevel1);
    			...
    			$this->sql= "select `NAME`,LINK FROM XT_MODEL WHERE  CID='".$this->getCID()."' AND POSITION(CONCAT(PRINAME) IN '".$pri2."') > 0 AND  PARENT = '".$id."'  AND `LEVEL` =2 ";
//     			echo $this->sql."<br>";
    			$stmt = $this->execute($this->sql);
    			$arrMenuList = array();
    			while ($rs = mysql_fetch_array($stmt)){
    ...
    public function getAllModel(){
    	...
    	if($modelList==""||!is_array($modelList||empty($modelList))){
    		...
    		foreach ($arr as $val){
    			...
    			$id = $a[0];
    			$name = $a[1];
    			$this->sql= "select `NAME`,PRINAME FROM XT_MODEL WHERE  CID='".$this->getCID()."'  and  PARENT = '".$id."'  AND `LEVEL` =2 ";
