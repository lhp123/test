...
var K = KindEditor.myready(); //获取 KindEditor对象
   ...
   // 查看图片 
		...
		$("#myform input[stype='showPhoto']").click(function() {
		    ...
		    var width,height;
		    var picms="";
		    var pictype=$(this).siblings("input[type='text']").attr("sname");
		    if(pictype=="PHOTO") 
		    	picms="室外图";
		    else if(pictype=="XQPHOTO")
		    	picms="小区图";
		    else if(pictype=="TITLE_PHOTO")
		    	picms=$(this).siblings("input[type='hidden'][sname='TITLE_PHOTO_MS']").val();
		    for(var i = 0;i<arr.length;i++ ){		    	
		    	body = body + "<li style='float:left;list-style: none;padding-left:5px;'><a href='"+arr[i]+"' target='_blank'><img src='"+arr[i]+
		    		"' width='200px' height='150px' /></a><br><span style='float:right;margin-right:20px;'>描述:<input type='text' name='photoms' value='"+
					picms+"' style='width:150px;'><br><a class='title' href='javascript:void(0);'>设为标题图</a>&nbsp;|&nbsp;<a class='del' href='javascript:void(0);'>删除</a></span></li>";
	    	}
		    var num = arr.length;
		    if(num<=1){
		    	height = 290;
				...
				$("#listphoto .title").click(function (){
					if(confirm("确定将当前图片设置为标题图?")) {
						$("#TITLE_PHOTO").val($(this).parents("li").find("img").attr("src"));		
						$("#TITLE_PHOTO_MS").val($(this).parents("li").find("input[name='photoms']").val());
						dialog.remove();
					}
				});
