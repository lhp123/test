...
*/
	function IpLocation($datfile = "QQWry.dat"){
		$this->fp=fopen(PATH_WEBROOT."include/".$datfile,'rb')or die("IP库不存在");   //二制方式打开
		...
		switch (ord($flag)) {
			case 1:  //地区1地区2都重定向
				...
				switch (ord($flag)) {
					case 2:  //地区1又一次重定向,
						fseek($this->fp,$this->getoffset());
						$address["area1"]=$this->gbk_utf8($this->getstr());
						fseek($this->fp,$address_offset+4);      //跳4个字节
						$address["area2"]=$this->gbk_utf8($this->readaddress());  //地区2有可能重定向,有可能没有
						break;
					default: //地区1,地区2都没有重定向
						fseek($this->fp,$address_offset);        //定位指针到重定向的地址
						$address["area1"]=$this->gbk_utf8($this->getstr());
						$address["area2"]=$this->gbk_utf8($this->readaddress());
						break;
				}
				break;
