...
<article class="jsqmain" style="width:74%; float:right;">
    ...
    <dl class="jsqtr01">
              <dt>公积金贷款利率<br>商业贷款利率</dt>
       <dd><?= $rate_gjj."%"?><br><?= $rate_sy."%"?></dd>
           </dl>
    <dl class="jsqtr01">
        <dt>按揭年数</dt>
    ...
    <div style="display:none" id="arrowshow">
            <?php 
                for ($i = 2; $i < $mortgage*12 ; $i++) {
                    $pay = 10000*$total_sy/$paymonth + 10000*$total_sy *$rate_month_sy *($paymonth-$i+1)/$paymonth +10000*$total_gjj/$paymonth + 10000*$total_gjj *$rate_month_gjj *($paymonth-$i+1)/$paymonth;
                    if($i<13){
    ...
    <div class="jsqbotton"><a href="fdjsq_zh.php"><input type="button" value="重新计算" ></a></div>
</article>
<!--计算器 end-->
...
<?include_once 'tail_jsq.php'; ?>owshow').hide();
        getPageSize();
    }
});
</script>
