...
function jjrsearch($conn,$cid,$page,$size,$JJRDEFPHOTO="img/peoType.png"){	
		...
		$condition="";
		$msql="SELECT @rownum:=@rownum+1 ROW,b.* FROM (SELECT @rownum:=0,D.DEPTNAME,U.* FROM XT_DEPT D,XT_USER U WHERE D.ID=U.FK_DEPTID ".$condition." AND D.IFDELETED=0 AND U.IFDELETED=0 AND U.CID='".$cid."' ORDER BY U.TABORDER limit ".($size*$page).",".$size.") b ";
		$mstmt=mysql_query(get_gbk($msql),$conn);
		while ( $mrs=mysql_fetch_array ( $mstmt ) ){	
			if($mrs["ROW"]==1){
				p_gbk("<div class='Gb2'>".($size*$page+1)."-".($size*($page+1))."</div>");//
			}
...
function JjrFyList($conn,$cid,$page,$size,$uid){
		...
		while ( $mrs=mysql_fetch_array ( $mstmt ) ){ 
			if($mrs["ROW"]==1){
				p_gbk("<div class='Gb4'>".($size*$page+1)."-".($size*($page+1))."</div>");//
			}
			echo "<a href='fydetail.php?id=".$mrs["ID"]."' class='Gb' style='background-color:#333;'> ";
			$photo=$mrs["PHOTO"];
			$photo=explode(";",$photo);
			echo "<img src='".getCompressPic($photo[0])."'> ";
			echo "<span class='Gb3'><span>".$mrs["TITLE"]."</span><span>".$mrs["DEPTNAME"]."&nbsp;&nbsp;<em class='b1b'>".$mrs["DISNAME"]."</em></span>";
			p_gbk("<span>".$mrs["H_SHI"]."室".$mrs["H_TING"]."厅&nbsp;&nbsp;".number_format($mrs["BUILD_AREA"],0)."平米<label class='b1a Gf1'><i>".
			p_gbk("<span>双花园南里三区 ， 2007年建 ， 板楼</span>").
			number_format($mrs["PRICE"],0)."万</i></label></span></span> </a>");
		}
		return;    
}
